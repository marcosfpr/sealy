...
fn main() -> anyhow::Result<()> {
	...
	let encoder = BFVEncoder::new_with_base(&ctx, 100)?;
	...
	let x_enc = encryptor.encrypt(&encoder.encode_float(&[x])?)?;
	let y_enc = encryptor.encrypt(&encoder.encode_float(&[y])?)?;
	...
	let sum_dec = encoder.decode_float(&sum_dec)?;
