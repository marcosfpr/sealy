...
impl PyPlaintextTensor {
	...
	fn new(ndarr: Vec<PyPlaintext>) -> PyResult<Self> {
		let batch = seal::Tensor(ndarr.iter().map(|x| x.inner.clone()).collect());
		Ok(Self {
			inner: batch,
		})
	}
...
pub struct PyCiphertextTensor {
	inner: seal::Tensor<seal::Ciphertext>,
}
...
pub struct PyTensorEncryptor {
	inner: seal::TensorEncryptor<seal::Asym>,
}
...
pub struct PyCKKSTensorEncoder {
	inner: seal::TensorEncoder<seal::CKKSEncoder>,
}
...
impl PyCKKSTensorEncoder {
	...
	fn new(ctx: &PyContext, scale: f64) -> PyResult<Self> {
		...
		let inner = seal::TensorEncoder::new(encoder);
		Ok(Self {
			inner,
		})
	}
...
pub struct PyCKKSTensorEvaluator {
	inner: seal::TensorEvaluator<seal::CKKSEvaluator>,
}
...
impl PyCKKSTensorEvaluator {
	...
	fn new(ctx: &PyContext) -> PyResult<Self> {
		...
		let inner = seal::TensorEvaluator::new(evaluator.inner);
