...
pub struct PyAsymmetricComponents {
	inner: seal::AsymmetricComponents,
}
...
impl PyAsymmetricComponents {
	...
	pub fn new(
		...
		Self {
			inner: seal::AsymmetricComponents::new(u.inner, e.inner, r.inner),
		}
	}
...
pub struct PyEncryptor {
	pub(crate) inner: seal::Encryptor<seal::Asym>,
}

#[pymethods]
impl PyEncryptor {
	/// Creates an Encryptor instance initialized with the specified seal::Context,
	/// public key, and secret key.
	///
	/// * `ctx` - The seal::Context
	...
	pub fn new(
		...
		let encryptor =
			seal::Encryptor::with_public_key(&ctx.inner, &public_key.inner).map_err(|e| {
