...
impl PyBFVEncoder {
	...
	pub fn new(ctx: &PyContext) -> PyResult<Self> {
		let encoder = seal::BFVEncoder::new(&ctx.inner).map_err(|e| {
...
pub struct PyCKKSEncoder {
	pub(crate) inner: seal::CKKSEncoder,
}

#[pymethods]
impl PyCKKSEncoder {
	/// Creates a CKKSEncoder. It is necessary that the encryption parameters
	/// given through the seal::Context object support it.
	#[new]
	pub fn new(
		ctx: &PyContext,
		scale: f64,
