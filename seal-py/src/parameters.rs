...
impl PySchemeType {
	...
	pub fn new(val: u8) -> Self {
		Self {
			inner: seal::SchemeType::from_u8(val),
		}
	}
...
pub struct PyEncryptionParameters {
	pub(crate) inner: seal::EncryptionParameters,
}
...
impl PyEncryptionParameters {
	...
	pub fn new(scheme: PySchemeType) -> PyResult<Self> {
		let params = seal::EncryptionParameters::new(scheme.inner).map_err(|e| {
	...
	pub const fn get_block_size() -> u8 {
		seal::EncryptionParameters::block_size()
	}
	...
	pub fn __setstate__(
		...
		self.inner = seal::EncryptionParameters::from_bytes(&self.get_scheme().inner, &state)
			.map_err(|e| {
				PyErr::new::<pyo3::exceptions::PyException, _>(format!(
...
impl PyCoefficientModulus {
	...
	pub fn create(
		...
		let modulus =
			seal::CoefficientModulusFactory::build(degree.inner, &bit_sizes).map_err(|e| {
				PyErr::new::<pyo3::exceptions::PyException, _>(format!(
	...
	pub fn bfv(
		...
		let modulus = seal::CoefficientModulusFactory::bfv(degree.inner, security_level.inner)
			.map_err(|e| {
				PyErr::new::<pyo3::exceptions::PyException, _>(format!(
					"Error creating CoefficientModulus: {}",
					e
	...
	pub fn max_bit_count(
		...
		seal::CoefficientModulusFactory::max_bit_count(degree.inner.into(), security_level.inner)
	}
}
...
impl PyPlainModulus {
	...
	pub fn batching(
		...
		let modulus = seal::PlainModulusFactory::batching(degree.inner, bit_size).map_err(|e| {
	...
	pub fn raw(val: u64) -> PyResult<PyModulus> {
		let modulus = seal::PlainModulusFactory::raw(val).map_err(|e| {
			PyErr::new::<pyo3::exceptions::PyException, _>(format!("Error creating Modulus: {}", e))
		})?;
		Ok(PyModulus {
			inner: modulus,
...
impl PyModulus {
	...
	pub fn new(value: u64) -> PyResult<Self> {
		let inner = seal::Modulus::new(value).map_err(|e| {
			PyErr::new::<pyo3::exceptions::PyException, _>(format!("Error creating Modulus: {}", e))
		})?;
		Ok(Self {
			inner,
...
impl PyDegreeType {
	...
	pub fn new(degree: u64) -> PyResult<Self> {
		let degree = seal::DegreeType::try_from(degree).map_err(|e| {
			PyErr::new::<pyo3::exceptions::PyException, _>(format!(
...
pub struct PySecurityLevel {
	pub(crate) inner: seal::SecurityLevel,
}
...
impl PySecurityLevel {
	...
	pub fn new(value: i32) -> PyResult<Self> {
		let level = seal::SecurityLevel::try_from(value).map_err(|e| {
			PyErr::new::<pyo3::exceptions::PyException, _>(format!(
	...
	pub fn default() -> Self {
		Self {
			inner: seal::SecurityLevel::default(),
		}
	}
