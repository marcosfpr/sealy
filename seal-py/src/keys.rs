...
pub struct PyPublicKey {
	pub(crate) inner: seal::PublicKey,
}
...
impl PyPublicKey {
	...
	pub fn new() -> PyResult<Self> {
		let pk = seal::PublicKey::new().map_err(|e| {
	...
	pub fn from_bytes(
		...
		let pk = seal::PublicKey::from_bytes(&context.inner, &bytes).map_err(|e| {
			PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
...
pub struct PySecretKey {
	pub(crate) inner: seal::SecretKey,
}
...
impl PySecretKey {
	...
	#[new]
	pub fn new() -> PyResult<Self> {
		let sk = seal::SecretKey::new().map_err(|e| {
			PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
	...
	pub fn from_bytes(
		...
		let sk = seal::SecretKey::from_bytes(&context.inner, &bytes).map_err(|e| {
			PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
...
pub struct PyRelinearizationKey {
	pub(crate) inner: seal::RelinearizationKey,
}
...
impl PyRelinearizationKey {
	#[new]
	pub fn new() -> PyResult<Self> {
		let rk = seal::RelinearizationKey::new().map_err(|e| {
			PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
	...
	pub fn from_bytes(
		...
		let rk = seal::RelinearizationKey::from_bytes(&context.inner, &bytes).map_err(|e| {
			PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
...
impl PyGaloisKey {
	#[new]
	pub fn new() -> PyResult<Self> {
		let gk = seal::GaloisKey::new().map_err(|e| {
			PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
	...
	pub fn from_bytes(
		...
		let gk = seal::GaloisKey::from_bytes(&context.inner, &bytes).map_err(|e| {
			PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
...
pub struct PyKeyGenerator {
	inner: seal::KeyGenerator,
}

#[pymethods]
impl PyKeyGenerator {
	/// Creates a KeyGenerator initialized with the specified seal::Context.
	#[new]
	pub fn new(ctx: &PyContext) -> PyResult<Self> {
		let gen = seal::KeyGenerator::new(&ctx.inner).map_err(|e| {
			PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
	...
	/// seal::Context and specified previously secret key.
	#[staticmethod]
	pub fn from_secret_key(
		ctx: &PyContext,
		secret_key: &PySecretKey,
	) -> PyResult<Self> {
		let gen = seal::KeyGenerator::new_from_secret_key(&ctx.inner, &secret_key.inner).map_err(
			|e| {
				PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
