...
pub struct PyPlaintext {
	pub(crate) inner: seal::Plaintext,
}
...
impl PyPlaintext {
	...
	pub fn new() -> PyResult<Self> {
		let plaintext = seal::Plaintext::new().map_err(|e| {
	...
	pub fn with_pool(memory: &PyMemoryPool) -> PyResult<Self> {
		let plaintext = seal::Plaintext::new_with_pool(&memory.inner).map_err(|e| {
			PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
	...
	pub fn from_bytes(
		...
		let plaintext = seal::Plaintext::from_bytes(&context.inner, &data).map_err(|e| {
			PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
	...
	pub fn from_hex_string(hex_str: &str) -> PyResult<Self> {
		let plaintext = seal::Plaintext::from_hex_string(hex_str).map_err(|e| {
			PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
