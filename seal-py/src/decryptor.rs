...
pub struct PyDecryptor {
	inner: seal::Decryptor,
}

#[pymethods]
impl PyDecryptor {
	/// Creates a Decryptor instance initialized with the specified seal::Context
	/// and secret key.
	#[new]
	pub fn new(
		ctx: &PyContext,
		...
		let decryptor = seal::Decryptor::new(&ctx.inner, &secret_key.inner).map_err(|e| {
