...
impl PyBFVEvaluator {
	...
	pub fn new(ctx: &PyContext) -> PyResult<Self> {
		let evaluator = seal::BFVEvaluator::new(&ctx.inner).map_err(|e| {
...
pub struct PyCKKSEvaluator {
	pub(crate) inner: seal::CKKSEvaluator,
}
...
impl PyCKKSEvaluator {
	...
	#[new]
	pub fn new(ctx: &PyContext) -> PyResult<Self> {
		let evaluator = seal::CKKSEvaluator::new(&ctx.inner).map_err(|e| {
			PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
